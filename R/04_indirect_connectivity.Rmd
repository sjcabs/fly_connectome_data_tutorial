...
Now, in this case, we are not interested in information that transers from Kenyon cells to dopamine neurons. Kenyon cells are the neurons the dopamine cell's "teach", and while their influence is important for the network it muddies the waters in terms if working out what sensory signals matter for teaching. We can do this as so:

```{r edgelist_filtered_no_kc}
mb_ids <-  meta %>%
  filter(grepl("^KC|^MBON|^APL|^DPM",cell_type)) %>%
  pull(!!sym(dataset_id))
edgelist_filtered_no_kc <- edgelist_filtered %>%
  filter(!pre %in% mb_ids, !post %in% mb_ids)
```
...
edgelist_cols <- colnames(edgelist_filtered_no_kc)
...
if ("pre" %in% edgelist_cols && "post" %in% edgelist_cols) {
  edgelist_for_ic <- edgelist_filtered_no_kc
} else {
...
# Build the abdominal_neuromere subset in code (no per-region cut-out folder in
# the new bucket). subset_by_region() reproduces the bancpipeline filter:
# neuropil matches `^ANm|^ABDNM` and ≥100 synapses per neuron.
dataset <- "banc_888"
dataset_id <- "banc_888_id"
subset_name <- "abdominal_neuromere"

# Read full meta and synapses (predicate-pushdown by neuropil keeps the download small)
meta_path    <- construct_path(data_path, dataset, "meta")
synapse_path <- construct_path(data_path, dataset, "synapses")
meta <- read_feather_gcs(meta_path, use_gcs = use_gcs)

if (!exists("gcs_fs")) gcs_fs <- if (use_gcs) setup_gcs_filesystem() else NULL
syn_pre_col  <- "pre_root_id"
syn_post_col <- "post_root_id"
if (use_gcs) {
  abd_syns <- query_parquet_gcs(
    path = synapse_path,
    gcs_filesystem = gcs_fs,
    filters = NULL,
    columns = c(syn_pre_col, syn_post_col, "neuropil")
  ) %>%
    rename(pre = all_of(syn_pre_col), post = all_of(syn_post_col)) %>%
    filter(grepl("^ANm|^ABDNM", neuropil))
} else {
  abd_syns <- open_dataset_lazy(synapse_path, format = "parquet") %>%
    select(all_of(c(syn_pre_col, syn_post_col)), neuropil) %>%
    rename(pre = all_of(syn_pre_col), post = all_of(syn_post_col)) %>%
    filter(str_detect(neuropil, "^ANm|^ABDNM")) %>%
    collect()
}

abd_sub <- subset_by_region(meta, dataset, subset_name, synapses = abd_syns)
abdominal_ids <- abd_sub$ids
abdominal_neurons <- abd_sub$meta
...
# lhns::lhn_odour_responses


```{lhns}
shahar.names1 <- rownames(lhn_odour_responses)
shahar.names2 <- gsub("nm20","",shahar.names1)
keep <- shahar.names2 %in% names(most.lhns)
lhn_odour_responses2 <- lhn_odour_responses[keep,]
shahar.names3 <- gsub("nm20","",rownames(lhn_odour_responses2))
lhn_odour_responses2 <- as.data.frame(lhn_odour_responses2)
lhn_odour_responses2$hemibrain_match <- unlist(most.lhns[shahar.names3,"hemibrain.match"])
hb.find <- neuprintr::neuprint_search("Traced",field="status",dataset="hemibrain:v1.2.1")
lhn_odour_responses2$hemibrain_cell_type <- unlist(hb.find$type[match(lhn_odour_responses2$hemibrain_match,as.character(hb.find$bodyid))])
fw.meta <- fafbseg::flytable_query("select cell_type, hemibrain_match, root_783 from info")
lhn_odour_responses2$cell_type <- unlist(fw.meta$cell_type[match(lhn_odour_responses2$hemibrain_match,as.character(fw.meta$hemibrain_match))])
lhn_odour_responses2 <- lhn_odour_responses2 %>%
