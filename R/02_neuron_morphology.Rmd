...
## Extension: Axon-Dendrite Splits

For many of our datasetss (but notably not BANC yet), we have axon-dendrite splits that have been pre-calculated using a graph-theoretic algorithm from [Schneider-Mizell et al. 2016](https://elifesciences.org/articles/12059).
...
vpn_ids <- vpn_meta[[dataset_fafb_id]]

# Construct path to fafb skeletons in BANC space
...
p<-plot3d(axon,col="orange")
p<-plot3d(dendrite,col="cyan")
p<-plot3d(linker,col="green")
plot3d(neurite,col="purple",soma=1000)
```
