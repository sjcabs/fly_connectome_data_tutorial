...
**GCS bucket location:** `gs://lee-lab_brain-and-nerve-cord-fly-connectome/compiled_data/`

### Option 2: Local Access (Faster for Repeated Use)
Download data once with `gsutil`, then access locally:

```bash
# Download specific dataset files (e.g., BANC metadata + synapses)
gsutil -m cp gs://lee-lab_brain-and-nerve-cord-fly-connectome/compiled_data/banc_888/banc_888_meta.feather ~/data/sjcabs_data/banc_888/
gsutil -m cp gs://lee-lab_brain-and-nerve-cord-fly-connectome/compiled_data/banc_888/banc_888_synapses_v2_enriched.parquet ~/data/sjcabs_data/banc_888/

# Or download an entire dataset directory
gsutil -m cp -r gs://lee-lab_brain-and-nerve-cord-fly-connectome/compiled_data/banc_888 ~/data/sjcabs_data/

# Or download all datasets (~500 GB total)
gsutil -m cp -r gs://lee-lab_brain-and-nerve-cord-fly-connectome/compiled_data ~/data/
```
...
gs://lee-lab_brain-and-nerve-cord-fly-connectome/compiled_data/manc_121/
gs://lee-lab_brain-and-nerve-cord-fly-connectome/compiled_data/hemibrain_121/
gs://lee-lab_brain-and-nerve-cord-fly-connectome/compiled_data/malecns_09/
```

The dataset folder name now includes the version (e.g. `banc_888`, not `banc`),
and the per-region "cut-out" folders that previously sat alongside the data files
(`mushroom_body/`, `antennal_lobe/`, etc.) have been removed. The tutorial
reproduces those subsets in code via the `subset_by_region()` helper.
...
```{r proofread}
# Use the metadata we already loaded
# Dataset ID - change this ID for different datasets, e.g. the BANC data is: banc_888_id
...
Synapse files can be very large (4-15 GB for full datasets). Earlier versions of this
tutorial shipped **pre-filtered Feather files** for common regions (one feather per
dataset/region pair). Those subset folders **no longer exist** — the new bucket only
stores the full Parquet synapse table for each dataset (e.g.
`compiled_data/banc_888/banc_888_synapses_v2_enriched.parquet`).

We rebuild the same subset in code by querying the Parquet file with a server-side
filter on `neuropil`. Parquet predicate pushdown means only rows matching the filter
are downloaded from GCS, which keeps this fast (typically under a minute for a
specific neuropil) and avoids loading the multi-GB full table.

### Loading Mushroom Body Calyx Synapses (Parquet predicate pushdown)
...
The BANC synapse Parquet (`banc_888_synapses_v2_enriched.parquet`) uses the
column names `pre_root_id` / `post_root_id` and stores `neuropil`, `region`,
and `side`. We pull only the rows whose `neuropil` starts with `MB_CA` (mushroom
body calyx).

```{r mb_synapses}
# Path to the full BANC synapse Parquet (no per-region cut-out file in the new bucket)
synapse_path <- construct_path(data_path, dataset, "synapses")

# BANC synapse columns: pre_root_id / post_root_id (other datasets use pre / post)
syn_pre_col  <- if (grepl("^banc", dataset)) "pre_root_id"  else "pre"
syn_post_col <- if (grepl("^banc", dataset)) "post_root_id" else "post"

if (use_gcs) {
  # GCS path: server-side filter via DuckDB / pyarrow predicate pushdown
  mb_synapses <- query_parquet_gcs(
    path = synapse_path,
    gcs_filesystem = gcs_fs,
    filters = "neuropil LIKE 'MB_CA%' AND side = 'right'",
    columns = c("id", syn_pre_col, syn_post_col, "neuropil", "side")
  )
} else {
  # Local path: Arrow lazy filtering
  mb_synapses <- open_dataset_lazy(synapse_path, format = "parquet") %>%
    filter(str_detect(neuropil, "^MB_CA"), side == "right") %>%
    select(id, all_of(c(syn_pre_col, syn_post_col)), neuropil, side) %>%
    collect()
}

# Normalise column names so downstream code can use `pre` / `post`
mb_synapses <- mb_synapses %>%
  rename(pre = all_of(syn_pre_col), post = all_of(syn_post_col)) %>%
  filter(pre %in% proofread_ids | post %in% proofread_ids)
...
# Use full dataset name with version for ID column (e.g., "banc_888_id")
mb_meta <- meta %>%
...
Consider:
- **BANC** = female fly, BANC project (CAVE materialization 626 → export 888)
